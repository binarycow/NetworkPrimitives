...
namespace NetworkPrimitives.Ipv6
{
    public readonly struct Ipv6SubnetMask : IEquatable<Ipv6SubnetMask>
    {
        ...
        public Ipv6Cidr ToCidr() => Ipv6Cidr.Parse(SubnetMaskLookups.GetCidr(this.High, this.Low));
        ...
        public static implicit operator Ipv6Cidr(Ipv6SubnetMask cidr) => cidr.ToCidr();
        ...
        internal static bool TryParse(ref ReadOnlySpan<char> text, ref int charsRead, out Ipv6SubnetMask result)
        {
            if (!TryParse(text, out var length, out result))
                return false;
            text = text[length..];
            charsRead += length;
            return true;
