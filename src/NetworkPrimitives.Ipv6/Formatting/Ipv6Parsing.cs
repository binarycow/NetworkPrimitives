...
namespace NetworkPrimitives.Ipv6
{
    ...
    {
        internal static bool TryParseIpv6Address(ReadOnlySpan<char> text, out int charsRead, out ulong high, out ulong low)
        {
            high = default;
            low = default;
            charsRead = default;
            var doubleColonPos = text.IndexOf("::".GetSpan());
            if (doubleColonPos == -1)
            ...
            static bool TryFillBitGroups(ReadOnlySpan<char> text, ref int charsRead, Span<ushort> bitGroups)
            {
            ...
            static int GetColonCount(ReadOnlySpan<char> text)
            {
                var ct = 0;
        ...
        internal static bool TryParseUnabbreviated(ReadOnlySpan<char> text, out int charsRead, out ulong high, out ulong low)
        {
            charsRead = default;
            Span<ushort> bitGroups = stackalloc ushort[8];
